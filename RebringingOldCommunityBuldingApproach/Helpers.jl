...
"""
    legacy_generate_equilibrium(A, R;
        margins_try = [margin, 0.75margin, 0.5margin, 0.25margin],
        rng = Random.default_rng(),
        k_cons::Float64 = 0.0,
        max_tries_per_margin::Int = 800,
        K_strategies = (:normal, :lognormal),
        K_normal_mu::Float64 = 2.0,
        K_normal_sigma::Float64 = 1.0,
        K_logn_mean::Float64 = 1.0,
        K_logn_cv::Float64   = 0.5
    ) -> (K, u, lambda_max)

Legacy-style feasible-threshold finder:
- Iterate over a *sequence* of stability margins (most stringent first).
- For each margin, try several K sampling strategies and many random draws.
- Accept iff u = (I - A) / K is positive and max Re eig(Diag(u)*(A - I)) ≤ -margin.
- No global shrink of A (alpha = 1.0).
"""
function legacy_generate_equilibrium(A::AbstractMatrix, R::Int;
    margins_try::AbstractVector{<:Real},
    rng = Random.default_rng(),
    k_cons::Float64 = 0.0,
    max_tries_per_margin::Int = 1200,
    K_strategies = (:normal, :lognormal),
    K_normal_mu::Float64 = 2.0,
    K_normal_sigma::Float64 = 1.0,
    K_logn_mean::Float64 = 1.0,
    K_logn_cv::Float64 = 0.5,
    throw_on_fail::Bool = false   # <--- added
)

    S = size(A,1)
    _strict_pos(v) = all(isfinite, v) && all(v .> 0.0)

    σK = sqrt(log(1 + K_logn_cv^2))
    μK = log(K_logn_mean) - σK^2/2

    K = similar(A, Float64, S)

    for m in margins_try
        for strategy in K_strategies
            scale_grid = (strategy === :normal) ? (0.6:0.2:1.6) : (1.0:1.0)
            for scale in scale_grid, _try in 1:max_tries_per_margin
                if strategy === :lognormal
                    K[1:R] = rand(rng, LogNormal(μK, σK), R)
                else
                    K[1:R] = abs.(rand(rng, Normal(K_normal_mu, K_normal_sigma), R)) .* (m * scale)
                end
                K[(R+1):S] .= k_cons

                u = (I - A) \ K
                if !_strict_pos(u); continue; end

                λ = maximum(real, eigvals(Diagonal(u) * (A - I)))
                if λ <= -m
                    return (K, u, λ)   # success
                end
            end
        end
    end
    ...
    if throw_on_fail
        error("legacy_generate_equilibrium: no feasible K for the given A after exhaustive search.")
    else
        return nothing
    end
end
