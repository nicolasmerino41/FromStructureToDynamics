...
function RunSimulations(;
    ...
    for idx in eachindex(combos)
        ...
        # Try stringent → lax margins (tune as you like)
        margins_try = (margin > 0) ? [margin, 0.75margin, 0.5margin, 0.25margin, 0.1margin, 0.05margin, 0.01] : [margin]

        found = legacy_generate_equilibrium(
            A0, R;
            margins_try = margins_try,
            rng = rng_local,
            k_cons = consumer_k,
            max_tries_per_margin = 1200,
            K_strategies = (:normal, :lognormal),
            K_normal_mu = 2.0, K_normal_sigma = 1.0,
            K_logn_mean = 1.0, K_logn_cv = 0.5,
            throw_on_fail = false
        )

        if found === nothing
            @info "Skipping combo: no feasible K (legacy-style skip)."
            continue
        end

        K0, u0, lambda_val = found
        alpha = 1.0  # legacy path never shrinks A

        ok, _ = check_equilibrium(u0, K0, A0; cb=cb)
        if !ok
            @info "Skipping combo: analytic u drifted numerically."
            continue
        end
...
@time sim_results = RunAllSimulations(;
    ...
    number_of_combinations_per_pair = 200,
    preserve_pair_symmetry = true
)
