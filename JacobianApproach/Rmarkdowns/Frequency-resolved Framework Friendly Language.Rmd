---
title: "Trajectory sensitivity to interaction uncertainty"
...
Rather than comparing two unrelated systems (original vs rewired, which confounds multiple effects), we focus on a single baseline system and quantify its sensitivity to uncertainty in interactions.
The central object is a frequency-resolved sensitivity spectrum derived from a generalized resolvent, which naturally links to Fourier modes and energy identities (Parseval theorem).

I do not aim to predict a unique time of a bump in time-domain differences.
Instead, we aim to identify which frequency bands (slow, intermediate, fast) concentrate the sensitivity to structural uncertainty, and then interpret how that spectral organization relates to recovery regimes in standardized time.
...
J = \mathrm{diag}(u)\, A.
$$
Equivalently, we define \(T = \mathrm{diag}(1/u)\), so that \(A = T J\) and \(J = T^{-1} A\).
In this representation, \(A\) encodes interaction structure (topology and interaction strength), while \(T\) encodes species time scales.

Throughout, we assume the baseline system is stable \(Resilience < 0\).

## Recovery and biomass weighting

We study recovery by using the biomass-weighted median return-rate.
Let \(C\) be a positive semidefinite weight matrix that weights perturbations by \(u^2\).
$$
C = \mathrm{diag}(u^2),
$$

We define the propagator \(E(t) = \exp(J t)\).
...
$$
Then the median return rate equals to
$$
r_{\mathrm{med}}(t) = -\frac{1}{2t}\, \log\Big( \frac{\mathcal{E}(t)}{\mathrm{tr}(C)} \Big).
$$

## Structural uncertainty defined as a perturbation

The key modeling step is to represent imperfect knowledge of interactions as a perturbation to \(A\).
We can write a perturbed interaction matrix as
$$
A_{\varepsilon} = A + \varepsilon P,
$$
where \(\varepsilon\) sets the uncertainty magnitude and \(P\) is the direction of the perturbation.

I suppose we can normalize \(P\) to separate direction from magnitude, for example
$$
\|P\|_F = 1,
$$
and choose \(\varepsilon\) based on the baseline system, for example proportional to \(\|\mathrm{offdiag}(A)\|_F\).
This would ensure that sensitivity comparisons across systems do not mix structure with perturbation size.

# New idea
I believe that considering a single P would cause misleading results since we do not know if it would excite the sensitive modes. In similar fashion to \(R_{med}\), we can have an ensemble of P that encodes the uncertainty model.
I don't really know how to build \(P\) but I find using \(\mathbb{P}(P)\) more correct. 
...
We consider harmonic forcing at angular frequency \(\omega\).
With \(T=\mathrm{diag}(1/u)\),  we define the generalized resolvent
$$
