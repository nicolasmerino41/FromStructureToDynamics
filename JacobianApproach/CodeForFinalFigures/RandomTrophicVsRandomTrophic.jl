...
using Distributions
using CairoMakie
using Distributions: HalfNormal
...
"""
Random trophic interaction matrix A:

- Off-diagonal interactions ONLY occur as predator–prey pairs (+/-).
- For an unordered pair {i,j}, with probability 'connectance' we create two directed effects:
    predator benefits from prey:   A[pred, prey] = +x
    prey is harmed by predator:    A[prey, pred] = -y
- x,y > 0 drawn from a positive distribution (default HalfNormal(σ)).

Note: directed connectance = expected fraction of nonzero off-diagonal entries = connectance.
"""
function random_trophic_matrix(S::Int, connectance::Real;
    σ::Real=1.0,
    rng=Random.default_rng(),
)
    A = zeros(Float64, S, S)
    for i in 1:S-1, j in i+1:S
        if rand(rng) < connectance
            # choose orientation: i predator j prey OR j predator i prey
            if rand(rng) < 0.5
                pred, prey = i, j
            else
                pred, prey = j, i
...
function run_pipeline(;
    ...
    for k in 1:n
        A = random_trophic_matrix(S, connectance; σ=σA, rng=rng)
        u = random_u(S; mean=u_mean, cv=u_cv, rng=rng)
        J = jacobian(A, u)
