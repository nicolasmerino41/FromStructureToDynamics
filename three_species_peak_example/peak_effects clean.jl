...
function figure_main(A,T,b,Ps,peaks,sims,ws)
    ...
    Label(fig[0,1:2],"Modifications A and B",fontsize=26,font=:bold)
    ax=Axis(fig[1,1],title="A  A slower third species receives both effects",limits=(-0.25,1.25,-0.2,1.2))
    ...
    axp=Axis(fig[1,2],title="B  Largest vertical gaps for ω ≥ 0.5",xlabel="Angular forcing frequency ω",ylabel="RMS departure from original",limits=((0.,6.),nothing))
    for l in 1:2
    ...
    rowsize!(fig.layout,1,Relative(.44));rowsize!(fig.layout,2,Relative(.56))
